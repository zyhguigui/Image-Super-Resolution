...
lIm = imresize(hIm, 1/upscale, 'bilinear');
lIm = imresize(lIm, size(hIm), 'bilinear');
[nrow, ncol] = size(hIm);
...
HP = zeros(patch_size^2, patch_num);    % Add on 2017/01/01
LP = zeros(4*patch_size^2, patch_num);  % Add on 2017/01/01
for ii = 1:patch_num    
    row = xrow(ii);
    col = ycol(ii);
