...
% L = size(X,1); % unused variable, 2017/01/03
% N = size(X,2); % unused variable, 2017/01/03
M = size(S, 1);
...
B_dual= Bt';
% fobjective_dual = fval_opt; % unused variable, 2017/01/03

B= B_dual;
% fobjective = fobjective_dual; % unused variable, 2017/01/03
% toc
...
function [f,g,H] = fobj_basis_dual(dual_lambda, SSt, XSt, ~, c, trXXt) % Replace X by ~, 2017/01/03
...
if nargout > 1   % fun called with two output arguments
    ...
    % g = zeros(M,1);     % Preallocation not recommanded, 2017/01/03
